...
class PDFIngestion:
    ...
    def build_vector_store(self, texts=None) -> Chroma:
        if texts is None:
            if self.chunks is None:
                self.split_documents()
            texts = self.chunks
        embeddings = SentenceTransformerEmbeddings(model_name=self.embedding_model)
        # Create/update collection using modern client if available
        try:
...
__all__ = ["PDFIngestion", "ingest", "find_pdf_paths", "load_documents", "split_documents", "create_vector_store"]
